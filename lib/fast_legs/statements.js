...
exports.select = function(model, selector, opts) {
  var fields = buildSelectFields(model, opts)
    , stmt   = "SELECT " + fields + " FROM " + utils.doubleQuote(model.tableName)
  ...
  var sql = stmt + join + where + order + limit + offset + ';';
  return sql;
};

exports.call = function(procedure, args, opts) {
  var fields = buildSelectFields(procedure, opts);
  var stmt = "SELECT " + fields + " FROM " + utils.doubleQuote(procedure) + '(' + buildCallArgs(args) + ')';
  return stmt + ';';
};

exports.insert = function(model, obj) {
  var stmt = "INSERT INTO " + utils.doubleQuote(model.tableName)
...
exports.update = function(model, selector, obj) {
  var stmt  = "UPDATE " + utils.doubleQuote(model.tableName)
...
exports.destroy = function(model, selector) {
  var stmt  = "DELETE FROM " + utils.doubleQuote(model.tableName)
    , where = buildWhereClause(model, selector);
...
exports.truncate = function(model, opts) {
  var opts = opts === undefined ? {} : opts
    , stmt = "TRUNCATE " + utils.doubleQuote(model.tableName);
...
exports.information = function(model) {
  var tableNameList = model.tableName.split('.');
  var tableName = tableNameList[tableNameList.length - 1];
  var stmt =  "SELECT column_name, is_nullable, data_type, " +
              "character_maximum_length, column_default " +
              "FROM information_schema.columns " +
              "WHERE table_name = '" + tableNameList + "';";
...
var buildJoinClause = function(model, opts) {
  if (_(opts.join).isUndefined()) {
    ...
    return  " INNER JOIN "  + opts.join[0] + " ON " +
            utils.doubleQuote(model.tableName) + "." + model.primaryKey + "=" +
...
var buildSelectFields = function(model, opts) {
  if (_(opts.only).isUndefined()) {
    if (!_(opts.join).isUndefined()) {
      return utils.doubleQuote(model.tableName) + ".*";
    } else {
      return "*";
    }
...
var buildWhereClause = function(model, selector) {
  if (utils.nil(selector)) {
    ...
    var pred = utils.doubleQuote(model.tableName) + '.' + model.primaryKey + " IN (" + ids + ")";
  } else if (_(selector).isNumber() || _(selector).isString()) {
    var id = selector;
    var pred = utils.doubleQuote(model.tableName) + '.' + model.primaryKey + " = '" + id + "'";
  } else {
    var pred =  _(selector).chain()
                .map(function(value, key) {
                  if (utils.fieldIsValid(model, key)) {
                    return utils.doubleQuote(model.tableName) + '.' + buildOperator(key, value);
                  } else {
